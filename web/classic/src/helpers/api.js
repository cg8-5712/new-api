...
import { BASE_PATH, getOriginWithBasePath } from './base-path';
...
export async function onDiscordOAuthClicked(client_id, options = {}) {
  ...
  if (!state) return;
  const redirect_uri = `${getOriginWithBasePath()}/oauth/discord`;
...
export async function onOIDCClicked(
  ...
  url.searchParams.set(
    'redirect_uri',
    `${getOriginWithBasePath()}/oauth/oidc`,
  );
...
export async function onCustomOAuthClicked(provider, options = {}) {
  ...
  try {
    const redirect_uri = `${getOriginWithBasePath()}/oauth/${provider.slug}`;
